...
PSD_PN_list = [-122.75, -115.22, -111.28, -111.03, -106.46, -101.48, -101.48]
PSD_AN_list = [-128.15, -124.06, -115.71, -116.92, -112.73, -115.21, -106.42]  # dBc/Hz
...
for PSD in PSD_PN_list_radHz:
    my_C = cmpt_bunch_length_correction_factor(sigma_phi, phase_noise=True)
...
for PSD in PSD_AN_list_radHz:
    my_C = cmpt_bunch_length_correction_factor(sigma_phi, phase_noise=False)
    print('correction factor due to bunch length = {}'.format(my_C))
...
print('sanity check {}'.format(np.array(dey_PN_list)*1e9))
